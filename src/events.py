...
def fmt_commit(cmt: Dict[str, Any]) -> str:
    ...
    message = cmt["message"]
    message = message[:MAX_COMMIT_LEN] + (".." if len(message) > MAX_COMMIT_LEN else "")
...
def fmt_last_commits(data: Dict[str, Any]) -> List[str]:
    ...
    if len(commits) <= MAX_COMMIT_LOG_LEN:
        ...
        last_line = "... and {} commit".format(irccolors.colorize(ellipsized, "royal"))
...
def handle_forward_push(irc: IrcConnection, data: Dict[str, Any]) -> None:
    ...
    irc.schedule_message(
        "{} {} pushed {} to {} ({}):".format(
            fmt_repo(data), author, num_commits, branch, short_gh_link(data["compare"])
        )
    )
...
def handle_issue(irc: IrcConnection, data: Dict[str, Any]) -> None:
    ...
    irc.schedule_message(
        "{} {} {} issue {}: {} ({})".format(repo, user, action, issue_num, title, link)
    )
